...
public class LoginPage extends ActionEngine {
  ...
  /*  public void login(String username, String password) {
        ...
        info("🔐 Forced FORM login (SSO disabled)");
        performFormLogin(username, password);
    }
    */
    /**
    ...
    public void performFormLogin(String username, String password) {
        ...
        try {
            ...
            WebElement usernameField = wait.until(
                    ExpectedConditions.elementToBeClickable(loc.getinputUserName())
            );
            ...
            // ===== PASSWORD (React / Fluent UI SAFE) =====
            WebElement passwordField = wait.until(
                    ExpectedConditions.elementToBeClickable(loc.getinputPassword())
            );
            ...
            // 🔥 Slow typing is mandatory
            for (char ch : password.toCharArray()) {
                passwordField.sendKeys(String.valueOf(ch));
                Thread.sleep(100);
            }

            // 🔥 Submit using ENTER (matches manual behavior)
            passwordField.sendKeys(Keys.ENTER);

            // ===== LOGIN RESULT VALIDATION (SAFE – NO EXCEPTION) =====
            ...
            if (loginSuccess) {
                info("✅ FORM login successful for user: " + username);
                if (test != null) test.pass("Login successful");
                return;
            }

            // ===== Handle login failure (toast / no navigation) =====
            By errorToast = By.xpath(
                    "//span[contains(text(),'An error occurred during login')]"
            );

            if (isPresent(errorToast, 5)) {
                error("❌ Login failed – application rejected credentials");
                if (test != null) test.fail("Login failed: Invalid credentials");
            } else {
                error("❌ Login failed – dashboard not loaded");
                if (test != null) test.fail("Login failed: Dashboard not loaded");
            }

            // IMPORTANT: DO NOT throw exception here
            // Let test fail gracefully

        } catch (Exception e) {
            error("❌ FORM Login encountered exception: " + e.getMessage());
            if (test != null) test.fail("Login exception: " + e.getMessage());
            // DO NOT rethrow → prevents invalid session id
        }
    }
